...
router.get('/:tournament_id', async (req, res) => {
  try {
    const tournament = await Tournament.findById(req.params.tournament_id).populate('profiles');
    if (!tournament) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if(existenceCheck){
        return res.status(500).json({ errors: [{ msg: 'tournament already exist,try another event name' }] });
      }
...
router.post('/:email/:event_name', auth, async (req, res) => {
  try { 
    ...
    const tournament = await Tournament.findOne({eventName:req.params.event_name});
    if(!tournament){
      return res.status(500).json({ errors: [{ msg: 'there is no such a tournament,try another event name' }] });
    }
    const player = await User.findOne({
      email: req.params.email.toString(),
    })
    if(!player){
      return res.status(500).json({ errors: [{ msg: 'this user does not exists' }] });
    }
    const playerProfile = await Profile.findOne({user:player._id}).populate('user','name')
    if(!playerProfile){
      return res.status(500).json({ errors: [{ msg: 'this user had no profile yet.' }] });
    }
    playerProfile.tournaments.push(tournament);
    playerProfile.tournaments[playerProfile.tournaments.length-1].playerName=playerProfile.user.name;
